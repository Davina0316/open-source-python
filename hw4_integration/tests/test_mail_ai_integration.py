...
def test_crawl_get_email_content(
    ...
    """
    Should fetch exactly the bodies and IDs returned by the fake client, and
    should propagate the `limit` argument unchanged.
    """
    bodies: List[str]
    ids: List[str]
...
def test_crawl_no_emails_returns_empty(
    ...
    class EmptyGmail(RealGmailClientImpl):
        def connect(self) -> bool:
            return True
        def get_emails_list(self, mailbox: str, limit: int) -> List[Dict[str, Any]]:
            return []
