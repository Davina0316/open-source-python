...
def test_should_connect_with_new_credentials(mock_build, mock_flow_factory, mock_file_open, mock_path_exists, client: GmailClientInterface):
    # Setup mock flow
    mock_flow = MagicMock()
    ...
    handle.write.assert_called_once_with('{"token": "test_token", "refresh_token": "test_refresh", "client_id": "test_client_id", "client_secret": "test_client_secret"}')
...
@patch("hw2_inbox.main.InstalledAppFlow.from_client_secrets_file")  # mock_flow_factory
@patch("hw2_inbox.main.build")  # mock_build
def test_should_connect_with_valid_token(mock_build, mock_flow_factory, mock_file_open, mock_creds_from_file, mock_path_exists, client: GmailClientInterface):
...
@patch("hw2_inbox.main.build")  # mock_build
def test_should_refresh_token_if_expired(mock_build, mock_file_exists, mock_creds_from_file, mock_file_open, mock_flow_factory, client: GmailClientInterface):
    # Setup mock credentials
    ...
    mock_file_open.assert_called_once()
    handle = mock_file_open()
    handle.write.assert_called_once_with('{"token": "refreshed_token", "refresh_token": "123", "client_id": "test_client_id", "client_secret": "test_client_secret"}')
...
def test_authenticate_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_connect_then_login_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials
    mock_creds = MagicMock(spec=Credentials, valid=True)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_login_failure_wrong_password(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials
    mock_creds = MagicMock(spec=Credentials, valid=True)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_logout_clears_state(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials
    mock_creds = MagicMock(spec=Credentials, valid=True)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_use_mailbox_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_use_mailbox_failure_invalid_mailbox(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_fetch_mailboxes_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_get_emails_list_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_get_email_content_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
    ...
    email = client.get_email_content('1')
    assert email['id'] == '1'
    assert email['subject'] == 'Test Subject'
    assert email['sender'] == 'sender@example.com'
    assert email['to'] == 'recipient@example.com'
    assert email['date'] == '2024-03-20'
    assert email['body'] == 'VGVzdCBlbWFpbCBib2R5'
    mock_messages.get.assert_called_once_with(userId='me', id='1', format='full')

@patch("hw2_inbox.main.Path.exists", return_value=True)
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_send_email_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_delete_email_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_mark_as_read_success(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
    ...
    mock_messages.modify.assert_called_once_with(userId='me', id='1', body={'removeLabelIds': ['UNREAD']})
...
@patch("hw2_inbox.main.Credentials.from_authorized_user_file")
@patch("hw2_inbox.main.build")
def test_operations_handle_api_errors(mock_build, mock_creds_from_file, mock_path_exists, client: GmailClientImpl):
    # Setup mock credentials (valid)
    mock_creds = MagicMock(spec=Credentials, valid=True, expired=False)
    mock_creds_from_file.return_value = mock_creds
