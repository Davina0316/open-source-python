...
def mock_client() -> Mock:
    return cast(Mock, create_autospec(GmailClientInterface, instance=True))

class TestGmailClientInterface:

    def test_connect(self, mock_client: Mock) -> None:
    ...
    def test_login(self, mock_client: Mock) -> None:
    ...
    def test_authenticate(self, mock_client: Mock) -> None:
    ...
    def test_logout(self, mock_client: Mock) -> None:
        mock_client.logout.return_value = None
        mock_client.logout()
        mock_client.logout.assert_called_once()

    def test_fetch_mailboxes(self, mock_client: Mock) -> None:
    ...
    def test_use_mailbox(self, mock_client: Mock) -> None:
    ...
    def test_get_email_content(self, mock_client: Mock) -> None:
    ...
    def test_send_email(self, mock_client: Mock) -> None:
    ...
    def test_delete_email(self, mock_client: Mock) -> None:
    ...
    def test_modify_email_labels(self, mock_client: Mock) -> None:
        mock_client.modify_email_labels.return_value = True
        result_add = mock_client.modify_email_labels("1", add_labels=["UNREAD", "STARRED"])
        assert result_add is True
        mock_client.modify_email_labels.assert_called_with("1", add_labels=["UNREAD", "STARRED"])

        mock_client.reset_mock() 
        mock_client.modify_email_labels.return_value = True
        result_remove = mock_client.modify_email_labels("2", remove_labels=["IMPORTANT"])
        assert result_remove is True
        mock_client.modify_email_labels.assert_called_with("2", remove_labels=["IMPORTANT"])

        mock_client.reset_mock() 
        mock_client.modify_email_labels.return_value = True
        result_both = mock_client.modify_email_labels("3", add_labels=["INBOX"], remove_labels=["UNREAD"])
        assert result_both is True
        mock_client.modify_email_labels.assert_called_with("3", add_labels=["INBOX"], remove_labels=["UNREAD"])

        mock_client.reset_mock() 
        mock_client.modify_email_labels.return_value = True 
        result_none = mock_client.modify_email_labels("4")
        assert result_none is True
        mock_client.modify_email_labels.assert_called_with("4")
